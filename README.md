...
| Query  | HMM_Files | HMM_Hits | Consensus_Coverage | I | Links |  |  |
| :-------------: | :-------------: | :-------------:| :-------------: | :-------------: | :------------- | :------------- | :------------- |
...
The default threshold is dynamically set according to the query sequence length. If the length of the query is below 150 aminoacids, the e-value is 1e-10, if above 150 and below 250, 1e-15, and if above 250, 1e-25.
This can however be set to a default value by adding `-et evalue`.
...
In the next step all the best hits are cycled through and extended by intersecting their metadata with the remaining hits' metadata. During this cycle, each best hit's metadata is checked against all the remaining hits' metadata, should there be a consensus between the two metadatas, the best hit's metadata is extended with the current hit's metadata.  

To calculate this consensus a two-fold approach was used:
...
We then try to find synonyms shared between two annotations’ tokens list. For each token within the first list we check if any of the tokens has a synonym which is present in the second list, if it is we replace the current token with the synonym.

###### Building TF-IDF vectors 

TF-IDF measures the importance of a token to a document in a corpus. In a simplistic manner:
...
In this manner we have the groups of hits (from different sources) that match between them. We now also check which identifiers match between the different his and then create groups of HMM hits that match whether via identifiers or free-text. When we have the different groups of consensuses, we then evaluate the quality of each group of consensuses.
We then order the groups by their score and, starting from the highest score group, create a final consensus by adding non-overlapping consensus groups into one.
...
Mantis parallelizes its inner workflows via Python’s multiprocessing module. It achieves this by having a global task queue that continuously releases tasks to workers. During each main task of any parallelized workflow, a certain number of workers are recruited (children python processes) which will execute and consume all the tasks within the queue. When a worker has finished its job, it will execute another task from the queue, until there are no more tasks to execute. If the queue is well balanced, minimal idle time (time spent waiting for workers to empty the task queue) can be achieved.  

For example, let’s imagine the annotation workflow:

![workflow_design](Images/workflow_design.png)

The annotation workflow has several main tasks (in grey) which must occur in a predetermined sequence. However, some of these main tasks also have sub-tasks (in blue) which can be parallelized, since they are entirely independent of each other - for example, splitting sample X and Y can happen simultaneously. In some occasions these sub-tasks can be further divided into smaller tasks (in green), which can also be parallelized - for example, running a hmmscan on a chunk from sample X with the database Y is independent from running a hmmscan on the same chunk but with the database W, therefore these tasks can happen simultaneously.  
This is the core reason why Mantis is efficient and scalable, even when running with immensely large search space due to large reference datasets and/or large amount of query sequences.
